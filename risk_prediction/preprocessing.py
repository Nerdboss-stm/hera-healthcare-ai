...
df = pd.read_csv(os.path.join(_dir, "..", "data", "raw", "human_vitals.csv"))
...
df["Calculated_MAP"] = (
    2 * df["Diastolic Blood Pressure"] + df["Systolic Blood Pressure"]
) / 3
print("Calculated_MAP", df["Calculated_MAP"])

# Pulse Pressure = systolic - diastolic
df["Calculated_Pulse_Pressure"] = (
...
df.to_csv(
    os.path.join(_dir, "..", "data", "processed", "cleaned_vitals.csv"), index=False
)
