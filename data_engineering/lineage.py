...
class DataLineageTracker:
    ...
    def add_node(
        ...
        node = LineageNode(
            table=table, column=column, dtype=dtype, pii=pii, description=description
        )
    ...
    def build_pipeline_lineage(self) -> None:
        ...
        for col in [
            ...
            self.add_node(
                "raw_input",
                col,
                dtype="str" if col in ("patient_id", "clinical_note") else "float",
                pii=pii,
                description=f"Raw patient {col}",
            )
        ...
        for entity_type in [
            ...
            self.add_edge(
                ...
                entity_type,
                "regex_extraction",
                f"pattern_match(clinical_note, '{entity_type}')",
                stage="ner",
            )
        ...
        self.add_edge(
            ...
            "disposition",
            "treatment_planning",
            "treatment_agent(diagnosis, history)",
            stage="agents",
        )
        ...
        self.add_edge(
            ...
            "retrieved_passages",
            "vector_search",
            "faiss.search(embed(note), top_k=3)",
            stage="rag",
        )
        self.add_edge(
            "reasoning_output",
            ...
            "retrieved_passages",
            "vector_search",
            "faiss.search(embed(diagnosis), top_k=3)",
            stage="rag",
        )
        ...
        for metric in [
            ...
            self.add_edge(
                ...
                metric,
                "llm_judge",
                f"evaluate_{metric}(source, summary)",
                stage="evaluation",
            )
            self.add_edge(
                "raw_input",
