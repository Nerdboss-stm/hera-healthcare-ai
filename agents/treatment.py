...
TREATMENT_PROTOCOLS = {
    "I21.9": {  # Acute Coronary Syndrome
        ...
        "medications": [
            ...
            Medication(
                ...
                "SL",
                "q5min x3",
                "Vasodilator / pain relief",
                "Hypotension, PDE5 inhibitor use",
            ),
            Medication(
                "Atorvastatin",
                "80mg",
    ...
    "I26.99": {  # Pulmonary Embolism
        ...
        "medications": [
            Medication(
                ...
                "q12h",
                "Anticoagulation",
                "CrCl <30, active bleeding",
            ),
            Medication(
                "Rivaroxaban",
                "15mg",
                "PO",
                "BID x21 days then 20mg daily",
                "DOAC transition",
                "CrCl <15, liver disease",
            ),
        ],
        "monitoring": [
            "Continuous SpO2",
            "Repeat CT-PA if worsening",
            "Renal function",
        ],
        "disposition": "admit",
    ...
    "I50.9": {  # Heart Failure
        ...
        "medications": [
            ...
            Medication(
                ...
                "PO",
                "BID",
                "Rate control / remodeling",
                "Decompensated HF, bradycardia",
            ),
        ],
    ...
    "A41.9": {  # Sepsis
        ...
        "medications": [
            ...
            Medication(
                ...
                "continuous",
                "Vasopressor",
                "First-line per SSC guidelines",
            ),
        ],
        "monitoring": [
            "Lactate q4h until normalizing",
            "MAP > 65 mmHg",
            "UOP > 0.5 mL/kg/hr",
            "CVP if central line",
        ],
    ...
    "I63.9": {  # Stroke
        ...
        "medications": [
            Medication(
                ...
                "10% bolus, 90% over 1h",
                "Thrombolysis",
                "Hemorrhage, recent surgery, INR>1.7",
            ),
            Medication(
                "Aspirin",
                "325mg",
        ...
        "monitoring": [
            "Neuro checks q15min x2h",
            "BP q15min during tPA",
            "CT 24h post-tPA",
        ],
        "disposition": "admit",
    ...
    "K35.80": {  # Appendicitis
        ...
        "medications": [
            ...
            Medication(
                "Ondansetron", "4mg", "IV", "q6h PRN", "Antiemetic", "QT prolongation"
            ),
        ],
...
class TreatmentAgent:
    ...
    def plan(self, ctx: PatientContext, dx: DiagnosticResult) -> TreatmentResult:
        ...
        safe_meds = self._check_contraindications(protocol["medications"], ctx)
        ...
        if ctx.age < 18:
            precautions.append("Pediatric patient — verify weight-based dosing")
    ...
    def _check_contraindications(
        ...
        for med in medications:
            allergic = any(
                allergy.lower() in med.name.lower() for allergy in ctx.allergies
            )
            if allergic:
                logger.warning(
    ...
    def _check_interactions(
        ...
        for med in proposed:
            for (drug_a, drug_b), warning in DRUG_INTERACTIONS.items():
                if drug_a.lower() in med.name.lower() and any(
                    drug_b.lower() in cm for cm in current_lower
                ):
                    warnings.append(f"{med.name}: {warning}")
                elif drug_b.lower() in med.name.lower() and any(
                    drug_a.lower() in cm for cm in current_lower
                ):
                    warnings.append(f"{med.name}: {warning}")
        return warnings
