...
class FHIRConverter:
    ...
    def parse_patient(resource: dict) -> dict:
        ...
        if resource.get("resourceType") != "Patient":
            raise ValueError(f"Expected Patient resource, got {resource.get('resourceType')}")
    ...
    def parse_bundle(cls, bundle: dict) -> dict:
        ...
        if bundle.get("resourceType") != "Bundle":
            raise ValueError(f"Expected Bundle resource, got {bundle.get('resourceType')}")
            ...
            "prediction": [{
                "outcome": {
                    "coding": [{
                        ...
                        "code": "high-risk" if prediction == "High Risk" else "low-risk",
                        "display": prediction,
                    }]
                },
                ...
                "qualitativeRisk": {
                    "coding": [{
                        ...
                        "code": "high" if risk_score > 0.7 else "moderate" if risk_score > 0.4 else "low",
                    }]
                },
            }],
            "basis": [
                {"display": f"{k}: {v}"} for k, v in features.items()
            ],
            "meta": {
                "profile": ["http://hl7.org/fhir/StructureDefinition/RiskAssessment"],
                "tag": [{"code": "hera-ai", "display": "Generated by HERA Healthcare AI"}],
            },
        }
    ...
    @classmethod
    def vitals_to_observations(
        cls, patient_id: str, vitals: dict
    ) -> list[dict]:
